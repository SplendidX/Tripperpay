...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeTripperpayAmount"))
        settings.setValue("nAnonymizeTripperpayAmount", 1000);
    nAnonymizeTripperpayAmount = settings.value("nAnonymizeTripperpayAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeTripperpayAmount"))
        SoftSetArg("-anonymizetripperpayamount", settings.value("nAnonymizeTripperpayAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeTripperpayAmount:
            return QVariant(nAnonymizeTripperpayAmount);
        default:
            return QVariant();
        }
    }
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeTripperpayAmount:
            nAnonymizeTripperpayAmount = value.toInt();
            settings.setValue("nAnonymizeTripperpayAmount", nAnonymizeTripperpayAmount);
            emit AnonymizeTripperpayAmountChanged(nAnonymizeTripperpayAmount);
            break;
        default:
            break;
        }
