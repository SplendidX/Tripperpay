...
void Shutdown()
{
    ...
    RenameThread("tripperpay-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: tripperpay.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: tripperpayd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizetripperpayamount=<n> " + _("Keep N Tripperpay anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Tripperpay is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Tripperpay is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Tripperpay version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Tripperpay server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Tripperpay") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Tripperpay to complete") << "\n";
    ...
    nAnonymizeTripperpayAmount = GetArg("-anonymizetripperpayamount", 0);
    if(nAnonymizeTripperpayAmount > 999999) nAnonymizeTripperpayAmount = 999999;
    if(nAnonymizeTripperpayAmount < 2) nAnonymizeTripperpayAmount = 2;
    ...
    LogPrintf("Anonymize Tripperpay Amount %d\n", nAnonymizeTripperpayAmount);
