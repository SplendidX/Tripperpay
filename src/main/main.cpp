...
const string strMessageMagic = "Tripperpay Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if (nHeight == 1) {
        nSubsidy = 1000000 * COIN; // TPR premine 5%
    }
    ...
    else if (nHeight > 400 && nHeight <= 43200) {
...
{
    int64_t nSubsidy = 0;
    if (pindexBest->nHeight+1 > 1 && pindexBest->nHeight+1 <= 43200) {
	nSubsidy = 10 * COIN;
    }
    else if (pindexBest->nHeight+1 > 43200 && pindexBest->nHeight+1 <= 150000)  {
        nSubsidy = 15 * COIN;
    }
    else if (pindexBest->nHeight+1 > 150001 && pindexBest->nHeight+1 <= 400000)  {
        nSubsidy = 15 * COIN;
    }
    else if (pindexBest->nHeight+1 > 400001 && pindexBest->nHeight+1 <= 600000) {
        nSubsidy = 10 * COIN;
    }
    else if (pindexBest->nHeight+1 > 600001 && pindexBest->nHeight+1 <= 800000) {
        nSubsidy = 8 * COIN;
    }
    else if (pindexBest->nHeight+1 > 800001 && pindexBest->nHeight+1 <= 1000000) {
        nSubsidy = 6 * COIN;
    }
    else if (pindexBest->nHeight+1 > 1000001 && pindexBest->nHeight+1 <= 1200000) {
	nSubsidy = 5 * COIN;
    }
    else if (pindexBest->nHeight+1 > 1200001 && pindexBest->nHeight+1 <= 1400000) {
	nSubsidy = 4 * COIN;
    }
    else if (pindexBest->nHeight+1 > 1400001 && pindexBest->nHeight+1 <= 1600000) {
	nSubsidy = 3 * COIN;
    }
    else if (pindexBest->nHeight+1 > 1600001 && pindexBest->nHeight+1 <= 1800000) {
	nSubsidy = 2 * COIN;
    }
    else if (pindexBest->nHeight+1 > 1800001 && pindexBest->nHeight+1 <= 2000000) {
	nSubsidy = 2 * COIN;
    }
    else if (pindexBest->nHeight+1 > 2000001 && pindexBest->nHeight+1 <= 2200000) {
	nSubsidy = 1 * COIN;
    }
    else if (pindexBest->nHeight+1 > 2200001 && pindexBest->nHeight+1 <= 2400000) {
	nSubsidy = 1 * COIN;
    }
    else if (pindexBest->nHeight+1 > 2400001) {
	// tipperpay PoS endblock - needs revising by developers prior to this
        nSubsidy = 1 * COIN;
    }
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        ...
        if (foundPaymentAndPayee) {
            ...
            CTripperpayAddress address2(address1);
...
{
    RenameThread("tripperpay-loadblk");
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue)
{
    ...
    if (nHeight < 43201) {
	ret = 0;
} else if (nHeight >= 43201 && nHeight <= 2400000) { //revise masternode gains after nHeight 2400000
        ret = blockValue * 11 / 14; // MN Reward 79%
    }
    return ret;
}
