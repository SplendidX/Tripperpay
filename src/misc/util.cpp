...
int nAnonymizeTripperpayAmount = 1000;
...
#else
    const char* pszModule = "tripperpay";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Tripperpay
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Tripperpay
    // Mac: ~/Library/Application Support/Tripperpay
    // Unix: ~/.tripperpay
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Tripperpay";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "tripperpay.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "tripperpayd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong TRIPPERPAY will not work properly.");
